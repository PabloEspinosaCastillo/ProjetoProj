...
exports.flipPlaca = async function (db, id, data) {
  if(data === null) return;
  // data.components.map(c => JSON.parse(c));
  ...
  // console.log('fdshjdsfhdsjkfhsdjfhsdjhfsdhf:', data.components.valueOf());

  const components = data.components.map(c => c.toString());
  console.log('COMPONENTS:', components);

  // await db.updateOne({ _id: new ObjectId(id) }, {$set: { components: components } });
  // await db.updateOne({ _id: new ObjectId(id) }, {$set: { components: data.components } });
...
exports.toPDF = (json) => {
  ...
  components.forEach(component => {
    ...
    doc.setFontSize(10);
    doc.text(`ID: ${component.id}`, 10, verticalOffset);
    doc.text(`X: ${component.x}`, 10, verticalOffset + 10);
    doc.text(`Y: ${component.y}`, 10, verticalOffset + 20);
    doc.text(`Rotation: ${component.rotation}`, 10, verticalOffset + 30);
    doc.text(`Type: ${component.type} ${component.outline}`, 10, verticalOffset + 40);
    doc.text('-------------------------------------------', 10, verticalOffset + 45);
